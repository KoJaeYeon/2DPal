...
public class Slot_Chest : MonoBehaviour
{
    ...
    {
        if (InventoryManager.Instance.tempSlot2.gameObject.activeSelf) // 잡고있는게 있으면
        {
    ...
    public void EndDrag()
    {
        if (!InventoryManager.Instance.tempSlot2.gameObject.activeSelf) return; // Temp 슬롯이 비 활성화일 때(인벤토리 꺼졌을때 드래그 유지 중)
        InventoryManager.Instance.tempSlot2.gameObject.SetActive(false);
