...
public class CraftDatabase : Singleton<CraftDatabase>
{
    ...
    {
        int index = 0;
        int id = 1001; Product product = new Product("팰 스피어", id, 1, 0,5, "던져서 팰을 포획하는 도구", new int[,] { { 1, 3 }, { 2, 3 }, { 3, 1 } }); product.sprite = sprites[index++]; Productions.Add(id, product);
        ...
        id = 1004; product = new Product("돌 곡괭이", id, 1, 10,30, "돌 곡괭이다.", new int[,] { { 1, 5 }, { 2, 5 } }); product.sprite = sprites[index++]; Productions.Add(id, product);
        id = 1005; product = new Product("돌 도끼", id, 1, 10,30, "돌 도끼다.", new int[,] { { 1, 5 }, { 2, 5 } }); product.sprite = sprites[index++]; Productions.Add(id, product);
    }
